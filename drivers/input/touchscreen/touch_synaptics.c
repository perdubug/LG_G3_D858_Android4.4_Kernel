...
static ssize_t show_firmware(struct i2c_client *client, char *buf)
{
	...
	if (!(strncmp(ts->fw_info.fw_image_product_id, "PLG313", 6)) || !(strncmp(ts->fw_info.fw_image_product_id, "PLG352", 6)) || !(strncmp(ts->fw_info.fw_product_id, "PLG391", 6))) {
		if (ts->fw_info.family && ts->fw_info.fw_revision) {
			ret += sprintf(buf+ret, "Touch IC : s3528(family_id = %d, fw_rev = %d)\n",
							ts->fw_info.family, ts->fw_info.fw_revision);
		} else {
...
static void set_knockOn_param(struct synaptics_ts_data *ts, int mode) //0: pocket,  1: origianl
{
	...
	touch_i2c_read(ts->client, wakeup_address, 6, udata);

	if (mode) {
		udata[2] = 0x0C;
		udata[4] = 0x05;
		udata[5] = 0x05;
		if (touch_i2c_write(ts->client, wakeup_address, 6, udata) < 0) {
			TOUCH_ERR_MSG("%s : Touch i2c write fail !! \n", __func__);
		}
	} else {
		...
		if (ts->lpwg_ctrl.qcover)
			...
			udata[2] = 0x06;
		udata[4] = 0x00; //timer1
		udata[5] = 0x05; //max activation timeout
		if (touch_i2c_write(ts->client, wakeup_address, 6, udata) < 0) {
			TOUCH_ERR_MSG("%s : Touch i2c write fail !! \n", __func__);
		}
	}
...
static int set_doze_param(struct synaptics_ts_data *ts, int value)
{
	...
	if (value == 1) {
		...
		touch_i2c_read(ts->client, f12_info.ctrl_reg_addr[27], 6, buf);
		max_active_duration = buf[3];
		...
		if (max_active_duration != buf[3])
			touch_i2c_write(ts->client, f12_info.ctrl_reg_addr[27], 6, buf);
	}
	TOUCH_DEBUG(DEBUG_BASE_INFO, "%s: state:[%d], [%d] [%d] [%d]\n", __func__, value, interval, wakeup, (int)buf[3]);
